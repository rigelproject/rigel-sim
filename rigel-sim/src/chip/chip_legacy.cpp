...
/// chip constructor
ChipLegacy::ChipLegacy(rigel::ConstructionPayload cp) :
  ComponentBase(cp.parent,
                cp.component_name.empty() ? "ChipLegacy" : cp.component_name.c_str()),
  _tiles(NULL),
  _global_cache(NULL),
  _bm(NULL),
  _gnet(NULL),
...
void 
ChipLegacy::init_tiles(rigel::ConstructionPayload &cp) {
...
void 
ChipLegacy::init_global_cache(rigel::ConstructionPayload &cp) {
...
void 
ChipLegacy::init_gnet(rigel::ConstructionPayload &cp) {
...
void 
ChipLegacy::init_bm(rigel::ConstructionPayload &cp) {
  _bm = new BroadcastManager(cp);
  cp.broadcast_manager = _bm;
}

void ChipLegacy::save_state() const {
  //for(int i = 0; i < rigel::NUM_GCACHE_BANKS; i++) {
